...
/*
* aca ira todo lo relacionado a la manipulacion visual del bitmap
* */
public class BitmapUtils {
    ...
    // convertimos un bitmap a entrada valida para el modelo
    public static ByteBuffer bitmap2bytebuffer(Bitmap bitmap, int inputSize, int colorChannels){
        // redimensionar imagen
        Bitmap resized = BitmapUtils.redimensionarBitmap(
                bitmap,
                inputSize,
                inputSize
        );
        ...
        ByteBuffer buffer = ByteBuffer.allocateDirect(
                inputSize * inputSize * colorChannels * 4
        );
    ...
    //********** MODIFICAR COLORES E INTENSIDAD DEL BITMAP *************************
...
*/
   ...
   // ************ ROTACION DE LA IMAGEN ********************
    ...
    private static Bitmap rotarBitmap(Bitmap bitmap, float grados){
        if(bitmap == null){
            return null;
        }
    ...
    public static Bitmap recortarBitmap(Bitmap bitmap, BoundingBox caja){
        int width = bitmap.getWidth();
        int height = bitmap.getHeight();

        // convertimos valores normalizados en pixeles reales
        int x = (int)((caja.centroX - caja.ancho / 2f) * width);
        int y = (int)((caja.centroY - caja.alto / 2f) * height);
        int w = (int)(caja.ancho * width);
        int h = (int)(caja.alto * height);
    ...
    public static Bitmap redimensionarBitmap(
            Bitmap bitmap,
            int ancho,
            int alto
    ){
